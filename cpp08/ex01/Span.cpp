...
unsigned int Span::shortestSpan() {
  ...
  long shortestspan = static_cast<long>(this->_nums[1]) - static_cast<long>(this->_nums[0]);
  for (unsigned int i = 1; i < this->_nums.size() - 1; i++) {
      long nowspan = static_cast<long>(this->_nums[i + 1]) - static_cast<long>(this->_nums[i]);
...
unsigned int Span::longestSpan() {
  ...
  std::sort(this->_nums.begin(), this->_nums.end());
  long diff = static_cast<long>(this->_nums.back()) - static_cast<long>(this->_nums.front());
