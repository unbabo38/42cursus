...
ClapTrap::ClapTrap() : _name("ClapDefault"), _hitPoints(10), _energyPoints(10), _attackDamage(0) {
...
void  ClapTrap::attack(const std::string& target){
  if (_hitPoints <= 0 || _energyPoints <= 0) {
	std::cout << "ClapTrap " << _name << " has no points left to attack" << std::endl;
	return ;
  }
  _energyPoints--;
...
void ClapTrap::beRepaired(unsigned int amount) {
	...
	if (amount > UINT_MAX - _hitPoints)
	    ...
	    _hitPoints += amount;
    std::cout << "ClapTrap " << _name << " repairs " << actualAmount << " points!" << std::endl;
	printStatus();
}
